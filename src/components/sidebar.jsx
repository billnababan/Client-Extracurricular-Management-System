...
const Sidebar = ({ children }) => {
  ...
  const { register: registerss, handleSubmit: handleSubmitss, formState, reset: resetss } = useForm();
  ...
  const onSubmit = async (data) => {
    ...
    try {
      await axios.patch(`${import.meta.env.VITE_API_URL}/updateProfile`, formData, {
        headers: {
  ...
  const resetPassword = async (data) => {
    try {
      await axios.patch(`${import.meta.env.VITE_API_URL}/resetPassword`, data);
      resetss();
    ...
    <>
      <div className="drawer sm:drawer-open">
        ...
        <div className="drawer-side">
          ...
          <div className="menu bg-cyan-500 min-h-full w-80 p-4 text-white relative">
            <div className="dropdown mx-auto text-center relative">
              <div className="avatar mx-auto my-5 cursor-pointer" tabIndex={0} role="button" onClick={toggleDropdown}>
                <div className="w-36 rounded-md mb-5">{getMe?.photo != null ? <img src={`http://localhost:4000/${getMe?.photo}`} /> : <img src="./../user.png" />}</div>
              </div>
              {dropdownOpen && (
                <div ref={dropdownRef} className="absolute left-1/2 transform -translate-x-1/2 -mt-3 w-48 bg-tertiary rounded-lg shadow-lg">
            ...
            <div className={`transition-all duration-300  ${dropdownOpen ? "mt-20" : "mt-0"}`}>
              {role === 1 ? (
                <>
                  <Link to="/" className="flex pl-10 hover:text-white bg-cyan-400 text-black duration-300 items-center h-8 mb-5 font-bold text-[17px]">
                    Dashboard
                  </Link>
                  <Link to="/ekskul" className="flex pl-10 hover:text-white bg-cyan-400 text-black duration-300 items-center h-8 mb-5 font-bold text-[17px]">
                    Daftar Ekskul
                  </Link>
                  <Link to="/siswa" className="flex pl-10 hover:text-white bg-cyan-400 text-black duration-300 items-center h-8  font-bold text-[17px]">
                    Daftar Data Siswa
                  </Link>
                </>
              ) : role === 0 ? (
                <>
                  ...
                  <Link to="/riwayat" className="flex pl-10 hover:text-white bg-cyan-400 text-black duration-300 items-center h-8  font-bold text-[17px]">
                    Riwayat Pendaftaran
                  </Link>
                </>
              ) : (
            ...
            <div className="absolute bottom-8 left-10">
              <button className="bg-gray-200 px-10 py-2 rounded text-black hover:bg-gray-400  duration-300" onClick={handleLogout}>
                Logout
              </button>
            </div>
          </div>
      ...
      <dialog id="my_modal_70" className="modal">
        <div className="modal-box bg-primary text-black max-w-xl flex flex-col gap-8">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5 w-full justify-center items-center rounded-xl">
            <input
              ...
              type="text"
              className={`input input-bordered w-full bg-primary border border-black placeholder:text-tertiary ${errors.username && "input-error"}`}
            ...
            <input
              ...
              type="text"
              className={`input input-bordered  w-full bg-primary border border-black placeholder:text-tertiary ${errors.Kelas && "input-error"}`}
              placeholder="Kelas"
            />
            {errors.Kelas && <span className="text-red-500 text-sm">{errors.Kelas.message}</span>}
            ...
            <input
              ...
              type="file"
              className={`input input-bordered py-2 w-full bg-primary border border-black placeholder:text-tertiary ${errors.photo && "input-error"}`}
              placeholder="photo"
            />
      ...
      <dialog id="my_modal_69" className="modal">
        <div className="modal-box bg-primary text-black max-w-xl flex flex-col gap-8">
          ...
          <form onSubmit={handleSubmitss(resetPassword)} className="flex flex-col gap-5 w-full justify-center items-center rounded-xl">
